...
static void buildPhaseTable() {
    for (int n = 0; n < 128; n++) {
        ...
        double inc = freq * 16777216.0 / SOFTSYNTH_SAMPLE_RATE;
...
// Statics
uint8_t SoftSynth::midi_status = 0;
uint8_t SoftSynth::midi_data[2] = {0, 0};
uint8_t SoftSynth::midi_data_pos = 0;
uint8_t SoftSynth::midi_expected = 0;
uint8_t SoftSynth::preset = 0;

SoftSynth::Voice SoftSynth::voices[SOFTSYNTH_MAX_VOICES];
uint8_t SoftSynth::channel_program[16];
uint8_t SoftSynth::channel_volume[16];
uint8_t SoftSynth::channel_expression[16];
uint8_t SoftSynth::channel_modulation[16];
uint8_t SoftSynth::channel_pan[16];
int16_t SoftSynth::channel_pitchbend[16];
...
void SoftSynth::feedByte(uint8_t b) {
    if (b & 0x80) {
        if (b >= 0xF8) return;
        if (b >= 0xF0) {
            midi_status = 0;
...
void SoftSynth::processMessage(uint8_t status, uint8_t d0, uint8_t d1) {
...
int SoftSynth::findVoice(uint8_t ch, uint8_t note) {
    for (int i = 0; i < SOFTSYNTH_MAX_VOICES; i++) {
...
int SoftSynth::allocVoice(uint8_t ch, uint8_t note) {
    for (int i = 0; i < SOFTSYNTH_MAX_VOICES; i++) {
...
SoftSynth::PatchParams SoftSynth::getPatch(uint8_t ch, uint8_t program) {
...
void SoftSynth::noteOn(uint8_t ch, uint8_t note, uint8_t vel) {
...
void SoftSynth::noteOff(uint8_t ch, uint8_t note) {
    int v = findVoice(ch, note);
    if (v >= 0) {
...
void SoftSynth::controlChange(uint8_t ch, uint8_t cc, uint8_t val) {
    switch (cc) {
...
void SoftSynth::programChange(uint8_t ch, uint8_t prog) {
    channel_program[ch] = prog;
}

void SoftSynth::pitchBend(uint8_t ch, int16_t bend) {
...
void __not_in_flash("softsynth") SoftSynth::gen_sound(uint8_t *buf_L, uint8_t *buf_R, int count) {
    for (int i = 0; i < count; i++) {
        ...
        for (int v = 0; v < SOFTSYNTH_MAX_VOICES; v++) {
            Voice &vc = voices[v];
