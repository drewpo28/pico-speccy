...
void act_gameScwong() {
    ...
    int pw = 3, bw = 4, bh = 4, pv_player = 4;
    int paddle_x = 0, plane_p = 0, plane_c = 0;
    UiColor colField = C_BG, colPad = C_ACCENT;
    auto applyOpts = [&]() {
    ...
    // ── the options page ───────────────────────────────────────────────────────
    static const char* const k_optRows[5] = {
        "Field colour", "Paddle colour", "Paddle width", "Ball size",
        "Paddle speed",
    };
    auto optValue = [&](int i) -> const char* {
        switch (i) {
            ...
            case 3:  return k_ballName[Config::gm_ball % 3];
    ...
    auto optCycle = [&](int i, int dir) {
        ...
        switch (i) {
            ...
            case 3:  step(Config::gm_ball,  3);                break;
    ...
    auto drawOptions = [&]() {
        ...
        // live preview under the list: a strip of field colour carrying a
        // paddle and the ball at their configured sizes
        const int sh = 26;
        const int sy = ry + 5 * lh + 6;
        ...
        fill(sx + 4 * sc, sy + (sh - 16) / 2, k_padW[Config::gm_padw % 3] * sc, 16,
        ...
        while (kbd->virtualKeyAvailable()) {
            ...
            switch (k.vk) {
                ...
                case fabgl::VK_a:    case fabgl::VK_A:
                    ...
                    else if (st == ST_OPTS && osel < 4) { osel++; drawOptions(); }
                    break;
                case fabgl::VK_LEFT:  case fabgl::VK_MENU_LEFT:
                    if (st == ST_OPTS) { optCycle(osel, -1); drawOptions(); }
                    break;
                case fabgl::VK_RIGHT: case fabgl::VK_MENU_RIGHT:
                    if (st == ST_OPTS) { optCycle(osel, +1); drawOptions(); }
                    break;
                case fabgl::VK_SPACE:
