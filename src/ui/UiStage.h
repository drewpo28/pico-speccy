...
#define NM_SETTINGS_TABLE(X)                                                              \
    ...
    X(SET_YM2413,          AC_SUBSYS, F_SUBSYS,              get_ym2413,     put_ym2413,     nullptr,        -1)          \
    X(SET_VGM_ALL,         AC_SUBSYS, F_SUBSYS,              get_vgmAll,     put_vgmAll,     nullptr,        -1)          \
    X(SET_CMS,             AC_SUBSYS, F_SUBSYS,              get_cms,        put_cms,        nullptr,        -1)          \
    X(SET_SN76489,         AC_SUBSYS, F_SUBSYS,              get_sn76489,    put_sn76489,    nullptr,        -1)          \
    X(SET_SN_CLOCK,        AC_LIVE,   0,                     get_snClock,    put_snClock,    hook_snClock,   -1)          \
    ...
    /* block, which is the whole apply.                                                */ \
    X(SET_UI_VGA_PAL,      AC_LIVE,   F_PREVIEW | F_PALETTE, get_uiVgaPal,   put_uiVgaPal,   hook_uiLook,    -1)          \
    ...
    /* TS-Conf render frameskip (Config::tsconf_render_skip): pure live, EndFrame   */ \
    /* reads it per frame. APPEND ONLY — the ids are persisted preferences' order.  */ \
    X(SET_TSCONF_RSKIP,    AC_LIVE,   0,                     get_tsconfRskip,put_tsconfRskip,nullptr,        -1)
