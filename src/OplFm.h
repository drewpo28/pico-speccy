...
Deliberate differences, all size or platform driven:
 ...
 - tl_tab (26 KB) and sin_tab (32 KB), which MAME builds into .bss at runtime,
   are precomputed const flash arrays (src/OplTabs.h, generated by
   tools/gen_opl_tabs.c with byte-for-byte the same math) as int16/uint16 —
   ~29.7 KB of flash, zero RAM.
...
private:
    struct Slot {
        ...
        uint32_t wavetable;       // waveform * SIN_LEN
    };
