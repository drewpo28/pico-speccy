...
// "Software MIDI" — the original lightweight procedural synthesizer (Config::midi
// == 3). 16 voices of square/saw/triangle/noise with ADSR, a 1-pole low-pass and
// 8 General-MIDI-ish presets (Config::midi_synth_preset). No sample data; needs
// no SD bank. The richer GM.DLS wavetable engine is the separate MidiSynth class
// (Config::midi == 4). Output: 8-bit unsigned, silence = 0, mixed into the L/R
// buffers by ESPectrum::loop.

#define SOFTSYNTH_MAX_VOICES 16
#define SOFTSYNTH_SAMPLE_RATE 31250
...
private:
    ...
    static Voice voices[SOFTSYNTH_MAX_VOICES];
