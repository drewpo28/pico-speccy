...
int16_t* ESPectrum::audioBufferOPL_L = nullptr;
    ...
    if (!maxSpeed) {
      ...
      if (Config::v_sync_enabled) {
        for (;;) {
          ...
          if (Z80Ops::isTsconf && VIDEO::tsPollWanted()) VIDEO::tsPalettePoll(false);
          if (v_sync) {
            v_sync = false;
            break;
          }
        ...
        if (Z80Ops::isTsconf && VIDEO::tsPollWanted()) VIDEO::tsPalettePoll(true);
      } else {
        if (idle > 0) {
          Debug::pumpUart();
          ...
          if (Z80Ops::isTsconf && VIDEO::tsPollWanted()) {
