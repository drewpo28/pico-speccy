...
// backlog at HDMI_AUDIO_RING_DROP samples. So occupancy is ~2 samples in
// steady state (a tick writes 1-2 and the same call encodes up to 8 out) and
// grows only while the PACKET queue is full — and everything above the drop
// cap is thrown away anyway, being >10 ms late. 256 with the cap at 192 keeps
// 64 samples of headroom over the deepest backlog that can survive. Was 1024,
// then 512 (2026-09), each step returning 1-2 KB of heap with no audible
// sample changed. Must stay a power of two (RING_MASK).
#define HDMI_AUDIO_RING_SIZE 256
#define HDMI_AUDIO_RING_DROP 192
#define HDMI_AUDIO_RING_MASK (HDMI_AUDIO_RING_SIZE - 1)
// Heap-allocated together with aq_blob (3072 B total) by hdmi_audio_init();
...
// Queue ENTRY: just the 4 stereo samples, 16 B. Everything else in an Audio
// Sample packet is derived — the header is two constants + the IEC block-start
// flags of a 192-frame counter, and each subpacket's zero bytes, parity bits and
// BCH are functions of the sample — so the consumer (hdmi_aq_expand, core1)
// rebuilds the 36-byte packet at pop time. Was 36 B per entry (2026-09-22:
// -2560 B of heap while HDMI audio is on), and the IEC counter moved to core1
// with it, where it has a single writer.
typedef struct { int16_t l[4], r[4]; } hdmi_aq_pkt_t;
static hdmi_aq_pkt_t *aq_blob = NULL;     // heap, see hdmi_audio_ring_L note
...
// line_bytes is even (400, or 408 in the 720-wide modes), so the A/B phase is identical on every line and every
...
// Companion diagnostic: longest time SPENT inside the ISR (µs). The gap counter
// above cannot show this — it only spikes once the margin is already gone, so it
// tells you the ISR was late, never how close to late it is. Nominal budget: the
// render lands two line periods (2 × 31.78 µs) ahead of the DMA that reads it,
...
static void __scratch_x("hdmi_driver") dma_handler_HDMI_body() {
    ...
    if (line < modep->v_active ) {
        ...
        // are cache-friendly; measured ~4.9x faster than the byte loop (~13 vs
        // ~2.6 cycles/pixel on M33), i.e. ~7.4 µs saved per 360-pixel line at
        // 504 MHz.
        //
        // The "margin went from ~1.3 µs to ~2.1 µs at 504 MHz" figures that used
        // to be here were wrong and self-contradicting — you cannot save 7.4 µs
        // out of a 1.3 µs margin — and no instrument in the tree could have
        // produced them (hdmi_irq_max_gap_us measures the ~31.8 µs inter-IRQ gap).
        // The real budget: this render lands two line periods (63.55 µs) ahead of
        // the DMA that reads it, of which the byte loop costs ~12.4 µs at 378 MHz
        // for 360 px. With HDMI audio the binding limit is instead the
        // `isr_gap < 45` Data-Island guard below. hdmi_irq_max_dur_us is the
        // counter to watch when changing anything in here.
...
// measurement. The body is static and called once, so it inlines here.
...
//    symbol bit b occupies bits [6b+2(b>=5) .. +5], so D0-7 = bits 0..49,
//    D9 = bits 56..61.
//
//  * left != right — that shortcut no longer applies and each channel needs its
//    OWN complement decision: complement the right pixel's data bits 0-7 when the
//    two halves have same-sign disparity on that channel. Channels with
//    opposite-sign disparity are already balanced. A single combined decision
...
// ============================================================
// TMDS level clamp (HDMI_TMDS_LEVEL_CLAMP)
//
...
//
// CLAMP keeps every channel inside [LO..HI], which moves the black and white ends
// off those worst patterns: worst swing 9 -> 7, worst run 11 -> 10. Costs ~3% of
// black level and ~4% of white (0x00 -> 0x08, 0xFF -> 0xF6) — the price of a stable
// link on a marginal receiver. hw-confirmed 2026-08-06 (pico-speccy + capture card,
// together with HDMI_SOFT_CLK); ported from pico-spec 43ea8c8, where it is
// confirmed on m1p1 + Samsung S27AG300N (yellow streaks over a white screen and a
// coloured left-edge column both gone).
//
// WHAT WAS TRIED AND REMOVED — HDMI_TMDS_LEVEL_SNAP (2026-08-06). pico-spec pairs
// CLAMP with a "snap": nudge each channel by ≤2 codes to the neighbour with the
// lowest (swing, run). On paper it is the bigger win of the two — measured here,
// mean swing 4.33 -> 3.44, ZX worst run 10 -> 6, Gigascreen worst swing 7 -> 5 —
// and the level shift is ≤2 codes, invisible. On hardware it broke the new UI both
// ways it was built:
//   * per-channel (pico-spec's form): each channel snaps independently and by a
//     different amount, so neutral greys pick up a cast (C_BG 0x0F1218 ->
//     0x101316 is R+1 G+1 B-2). It also quantises hard — 154 of the 255 steps
//     collapse, 0x1A..0x1E all become 0x1C. src/ui's palette is 16 closely-spaced
//     neutral greys, the one thing in this firmware that cannot absorb either.
//   * spread-aware (a shared offset for near-neutral colours, per-channel for
//     saturated ones): measured hue drift 0 on every near-neutral colour of every
//     palette this firmware installs, keeping nearly all of the signal win. The UI
//     was STILL broken on hw — and not as a colour fault: entering the menu left
//     the guest frame on screen with noise in vertical bands, i.e. the menu never
//     painted. No mechanism was found by which a legal 8-bit level value per
//     channel could do that (the pair construction, the palette indices and
//     palette[]/VGA all stay identical), so the cause is unexplained and was NOT
//     colour.
// Removed rather than left switchable: an option that breaks the menu for reasons
// nobody understands is worse than the signal margin it buys. Reinstate only with
// that mechanism in hand — the maths is in git history (this file, 2026-08-06), and
// re-measure it, because those numbers belong to THIS pair construction.
//
...
// logic see the exact colour. The CRT grille taps are derived from the clamped
// value and keep their own residual balancing (hdmi_crt_tap / hdmi_balanced_near
// are NOT re-clamped), so the grille loses none of its depth. Sync, porch and
...
static inline uint8_t hdmi_tmds_level(uint32_t v) {
    return (v < HDMI_TMDS_LEVEL_LO) ? HDMI_TMDS_LEVEL_LO
         : ((v > HDMI_TMDS_LEVEL_HI) ? HDMI_TMDS_LEVEL_HI : (uint8_t)v);
}

// Condition a whole RGB888 colour on its way into the TMDS encoder.
static inline uint32_t hdmi_tmds_level888(uint32_t c) {
    return ((uint32_t)hdmi_tmds_level((c >> 16) & 0xff) << 16)
         | ((uint32_t)hdmi_tmds_level((c >>  8) & 0xff) <<  8)
         |  (uint32_t)hdmi_tmds_level( c        & 0xff);
}
#else
...
static inline bool hdmi_palette_slot_writable(uint8_t i) {
    ...
    if (hdmi_audio_enabled &&
        ((i >= IDX_DI_DATA2_BASE && i < IDX_DI_DATA2_BASE + 16) ||
         (i >= IDX_DI_PREAMBLE_VS && i <= IDX_DI_GUARD_TRAIL_VS))) return false;
    return true;
}
...
void graphics_set_palette(uint8_t i, uint32_t color888) {
    ...
    if (!hdmi_palette_slot_writable(i)) return; //не записываем "служебные" цвета
    ...
    // Grille applies here rather than only in a one-shot walk because ULA+ rewrites
...
// CRT aperture grille level (0=Off, 1..3). Stores the level and re-emits the
// cached palette so the change is live — the same shape as vga_set_scanlines().
//
// Deliberately re-emits conv_color directly instead of going through
// graphics_set_palette(): that would also drive vga_set_palette_entry(), which
// clears vga_color_solid[] and would start dithering the 16 ZX colours. vga_set_crt()
// does its own solid-aware walk.
//
// DS80 is not re-emitted here: conv_color holds the pair table while DS80 is
...
void hdmi_set_crt(uint8_t level) {
    ...
    for (int i = 0; i < 256; i++)
        if (hdmi_palette_slot_writable((uint8_t)i)) hdmi_emit_slot((uint8_t)i, palette[i]);
}
...
static void hdmi_build_vendor_if_blob(void) {
    ...
    sp[0][3] = 0x00;
    hdmi_if_checksum(hdr, sp);
    memcpy(if_vendor.hdr, hdr, 4); memcpy(if_vendor.sp, sp, 32);   // raw, encoded per frame by hdmi_di_load
}
...
// IEC 60958 192-frame block counter. CONSUMER-side (core1, hdmi_aq_expand)
// since the queue went raw: it must advance once per packet actually emitted,
// and core1 is where that happens. PICO-BK convention: frame_index = (192 - ct) & 191.
static int hdmi_iec_frame_ct = 0;

// Producer: take the next 4 stereo samples off the ring into a queue entry.
// Runs on core0 in the 31250 Hz pcm timer IRQ (every ~2.6th tick). The packet
// framing (header, parity, BCH) is built by the consumer — see hdmi_aq_expand.
static void __not_in_flash_func(hdmi_encode_audio_blob)(hdmi_aq_pkt_t *out) {
    uint32_t rd = hdmi_audio_rd;
    for (int s = 0; s < 4; s++) {
        out->l[s] = hdmi_audio_ring_L[(rd + s) & HDMI_AUDIO_RING_MASK];
        out->r[s] = hdmi_audio_ring_R[(rd + s) & HDMI_AUDIO_RING_MASK];
    }
    hdmi_audio_rd = rd + 4;
}

// Consumer: rebuild the Audio Sample packet (4 samples L+R, 16-bit) from a
// queue entry. ~40 RAM table lookups (4 x bch7 + bch3 + 8 parity) on the
// core1 line ISR, once per audio line — a fraction of a microsecond against
// the 17-19 us the ISR already takes; every table and helper it touches is
// RAM-resident (the hdmi_isr_mode rule).
// IEC 60958 channel status byte 3 (sampling frequency) for HDMI_AUDIO_FS.
#define HDMI_CS_RATE ((HDMI_AUDIO_FS) == 32000 ? 0x03 : (HDMI_AUDIO_FS) == 44100 ? 0x00 : \
                      (HDMI_AUDIO_FS) == 48000 ? 0x02 : (HDMI_AUDIO_FS) == 96000 ? 0x0A : 0x01)
static void __not_in_flash_func(hdmi_aq_expand)(const hdmi_aq_pkt_t *in, hdmi_audio_pkt_t *out) {
    ...
    hdr[3] = hdmi_bch3(hdr);
    for (int s = 0; s < 4; s++) {
        const int16_t l = in->l[s], r = in->r[s];
        // IEC 60958 channel status, one C bit per frame (V=0, U=0). It used to be
        // all zeros ("exactly what SpeccyP transmits"), which is NOT "unspecified":
        // byte 0 bit 2 = 0 is copyright ASSERTED, byte 3 = 0 is 44.1 kHz (our ACR
        // and Audio InfoFrame say 48), byte 4 = 0 no word length. Now what the HSTX
        // test card sends: byte 0 = 0x04 (consumer LPCM, copy permitted), byte 3 =
        // the sample rate code, byte 4 = 0x02 (16-bit word). Frame f of the block:
        // ct counts down to the block start at 0, so f = (192 - ct) % 192; the only
        // set bits are f = 2 (byte 0 bit 2), 24 + rate bits, 33 (byte 4 bit 1).
        const int f = ct ? 192 - ct : 0;
        const int cs = (f == 2) || (f >= 24 && f < 32 && ((HDMI_CS_RATE >> (f - 24)) & 1)) || (f == 33);
        uint8_t *d = out->sp[s];
        ...
        // C on both channels (bits 2 and 6); P is even parity over the bits it
        // covers, so a set C flips it too — hence 0xCC.
        d[6] = (uint8_t)(((pl << 3) | (pr << 7)) ^ (cs ? 0xCC : 0x00));
        d[7] = hdmi_bch7(d);
        if (--ct < 0) ct = 191;
    }
    ...
    if (vs) {
        if (hdmi_au_pos >= (4u << 24)) {
            ...
            if (qd != 0 && qd <= HDMI_AQ_LEN) {
                ...
                hdmi_pack_blob_ch0(chars, NULL, apkt.hdr, apkt.sp, di_ch0_data_vs, HDMI_BLOB_SW);
                __dmb();
                aq_rd = aq_rd + 1;
                return true;
            }
...
// Speed Test: the timing the line ISR really runs (the snapshot, v_total included
// as last published by hdmi_update_mode_timing) and the frame count it keeps.
void hdmi_live_mode(struct video_mode_t *out) { *out = hdmi_isr_mode; }
uint32_t hdmi_frames(void) { return hdmi_frame_ct; }
...
bool hdmi_audio_init(void) {
    ...
    if (!hdmi_audio_enabled) {
        if (!aq_blob) {
            // Sample queue + both sample rings in one block (3072 B):
            // 128x16 B queue (raw samples, framed at pop) + 2x256x2 B rings.
            // Paid only while HDMI audio is enabled.
            // tryCalloc (src/TryAlloc.h), NOT calloc: pico_malloc PANICS on NULL,
            // so the failure branch below was dead code — hw 2026-09-21, a 576p +
            // TS-Conf + NeoGS boot with 17 KB free but no 6.6 KB hole died here
            // ("*** PANIC *** Out of memory" after "AY init done"). Declining
            // leaves HDMI audio off for the session, which is the right trade.
            extern void *tryCalloc(size_t n);
            uint8_t *blk = (uint8_t *)tryCalloc(HDMI_AQ_LEN * sizeof(hdmi_aq_pkt_t)
                                                + 2 * HDMI_AUDIO_RING_SIZE * sizeof(int16_t));
            if (!blk) return false;
            aq_blob = (hdmi_aq_pkt_t *)blk;
            hdmi_audio_ring_L = (volatile int16_t *)(blk + HDMI_AQ_LEN * sizeof(hdmi_aq_pkt_t));
...
#if HDMI_LIVE_AUDIO_DIAG
// Sample range handed to the HDMI path since the last read (min L, max L, min R,
// max R): says whether the packets carry signal at all, which pkt/s cannot.
static volatile int16_t hdmi_au_lvl[4] = { 32767, -32768, 32767, -32768 };
void hdmi_audio_levels(int16_t out[4]) {
    for (int i = 0; i < 4; i++) out[i] = hdmi_au_lvl[i];
    hdmi_au_lvl[0] = 32767; hdmi_au_lvl[1] = -32768; hdmi_au_lvl[2] = 32767; hdmi_au_lvl[3] = -32768;
}
#endif
...
void __not_in_flash_func(hdmi_audio_write_sample)(int16_t left, int16_t right) {
    ...
    if (right < hdmi_au_lvl[2]) hdmi_au_lvl[2] = right;
    if (right > hdmi_au_lvl[3]) hdmi_au_lvl[3] = right;
#endif
