...
// PICO-SPEC PATCH: the four bus dividers were computed inline in
// pio_usb_host_init() from the clk_sys value present at init time, and
// configure_root_port() re-applies them to the state machines on every SOF frame.
// pico-speccy brings the host up early (the USB-stick root volume and the CDC/ZiFi
// bridge are needed inside ESPectrum::setup()) but applies the saved Config::cpu_mhz
// afterwards, so the derivation is factored out here and called again from main.cpp
// after that clock switch. Without it the bus keeps clocking at the boot bit rate
// and nothing enumerates. Callable at any time; the SM clkdivs follow next frame.
void pio_usb_host_reclock(void) {
...
usb_device_t *pio_usb_host_init(const pio_usb_configuration_t *c) {
  ...
  pio_usb_host_reclock();   // PICO-SPEC PATCH: was inline here
...
// PICO-SPEC PATCH: read out one endpoint's internal state for diagnostics. Lets the
// firmware tell apart "device is NAKing, so our key state is simply stale" from "the
// transfer is armed but never completes" and "the arm was lost" — the three ways a USB
// keyboard can appear to hang, which need three different recoveries.
// Packed: bit0 has_transfer, bit1 transfer_started, bit2 transfer_aborted, bit3 stalled,
// bit4 need_pre, bits8..15 failed_count, bits16..31 actual_len. ~0 = endpoint not found.
uint32_t pio_usb_host_ep_debug(uint8_t dev_addr, uint8_t ep_num) {
  for (int i = 0; i < PIO_USB_EP_POOL_CNT; i++) {
    endpoint_t *ep = PIO_USB_ENDPOINT(i);
    if (!ep->size || ep->dev_addr != dev_addr) continue;
    if (ep_num == 0xFF) {
      // "any interrupt IN endpoint of this device" — the caller does not know the
      // endpoint address (TinyUSB keeps it private to the HID driver).
      if (!(ep->ep_num & 0x80) || ((ep->attr & 0x03) != EP_ATTR_INTERRUPT)) continue;
    } else if (ep->ep_num != ep_num) {
      continue;
    }
    return (uint32_t)(ep->has_transfer ? 1u : 0u) |
           (uint32_t)(ep->transfer_started ? 2u : 0u) |
           (uint32_t)(ep->transfer_aborted ? 4u : 0u) |
           (uint32_t)(ep->stalled ? 8u : 0u) |
           (uint32_t)(ep->need_pre ? 16u : 0u) |
           ((uint32_t)ep->failed_count << 8) |
           ((uint32_t)ep->actual_len << 16);
  }
  return ~0u;
}
...
// PICO-SPEC PATCH: how much of the 1 ms frame extra bulk transactions may consume.
// One 64-byte full-speed transaction (token + data + handshake + turnarounds) is
// ~55 us, so 700 us fits ~11 of them and still leaves 300 us for the next SOF to go
// out on time — a late SOF is a framing error for the attached devices.
// 700 us measured ~9.4 packets/frame (~590 KB/s of raw traffic) on hw, but it leaves
// the bit-banged bus busy for 70% of every frame — with a hub carrying BOTH a flash
// stick and a keyboard receiver that was enough for the receiver to stop answering
// altogether after a big read (hw 2026-07-31). 400 us still gives ~5x over the stock
// one-packet-per-frame behaviour while leaving the frame mostly idle. Raise it if the
// PIO port carries storage only.
#define PIO_USB_BULK_BUDGET_US 400
uint32_t pio_usb_bulk_extra_xacts = 0;   // diagnostics: extra transactions issued
...
void __not_in_flash_func(pio_usb_host_frame)(void) {
  ...
  const uint32_t frame_start_us = get_time_us_32();   // PICO-SPEC PATCH
  pio_port_t *pp = PIO_USB_PIO_PORT(0);
  ...
  // PICO-SPEC PATCH: bulk throughput.
  // Upstream services every endpoint exactly ONCE per 1 ms frame, so a bulk transfer
  // moves at most one 64-byte packet per frame: a hard ~64 KB/s ceiling, which is why
  // a USB stick on the PIO port ran ~10x slower than on the native controller. USB
  // imposes no such limit — a host may issue as many bulk transactions per frame as
  // fit. Keep going for NON-periodic endpoints while the frame budget lasts; interrupt
  // endpoints are deliberately untouched (their interval is part of the contract).
  // An endpoint that makes no progress (NAK / retry) is dropped for the rest of this
  // frame so a NAKing device cannot eat the whole budget.
  {
    uint32_t idle_mask = 0;   // bit per endpoint: no progress in this frame, skip
    bool progressed = true;
    while (progressed &&
           (get_time_us_32() - frame_start_us) < PIO_USB_BULK_BUDGET_US) {
      progressed = false;
      for (int root_idx = 0; root_idx < PIO_USB_ROOT_PORT_CNT; root_idx++) {
        root_port_t *root = PIO_USB_ROOT_PORT(root_idx);
        if (!(root->initialized && root->connected && !root->suspended)) {
        ...
        for (int ep_pool_idx = 0; ep_pool_idx < PIO_USB_EP_POOL_CNT; ep_pool_idx++) {
          if (idle_mask & (1u << ep_pool_idx)) continue;
          endpoint_t *ep = PIO_USB_ENDPOINT(ep_pool_idx);
          if (ep->root_idx != root_idx || !ep->size) continue;
          if ((ep->attr & 0x03) == EP_ATTR_INTERRUPT) continue;
          if (!ep->has_transfer || ep->transfer_aborted) continue;
          if ((get_time_us_32() - frame_start_us) >= PIO_USB_BULK_BUDGET_US) break;

          const uint16_t before = ep->actual_len;
          ep->transfer_started = true;
          if (ep->need_pre) pp->need_pre = true;

          if (ep->ep_num == 0 && ep->data_id == USB_PID_SETUP) {
            usb_setup_transaction(pp, ep);
          } else if (ep->ep_num & EP_IN) {
            usb_in_transaction(pp, ep);
          } else {
            usb_out_transaction(pp, ep);
          }
          ...
          if (ep->actual_len != before) {
            pio_usb_bulk_extra_xacts++;
            progressed = true;
          } else {
            idle_mask |= (1u << ep_pool_idx);   // NAK / error: leave it for next frame
          }
        }
      }
    }
