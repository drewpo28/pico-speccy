...
// PICO-SPEC PATCH: bound the TX-side spin loops.
//
// Upstream waits forever on the TX state machine's IRQ/PC flags. All of that code
// runs inside the 1 ms SOF timer IRQ, so a state machine that never completes (bus
// glitch, aborted DMA, a stall while the SM is disabled) freezes the WHOLE firmware
// with interrupts held — observed as "works, then hangs at random". The waits below
// get a ceiling instead: a full-speed 64-byte data packet is ~46 us on the wire and
// a low-speed 8-byte one ~60 us, so PIO_USB_TX_WAIT_US is over an order of magnitude
// of headroom. On expiry we simply return; the transaction then fails its CRC /
// handshake check and TinyUSB retries it, exactly like a NAK'd or corrupted packet.
// pio_usb_tx_timeouts counts the events for diagnostics (main.cpp logs changes).
#define PIO_USB_TX_WAIT_US 500
volatile uint32_t pio_usb_tx_timeouts = 0;

// Variant that must NOT return early: send_pre() has to finish switching the bus
// from full-speed (the PRE token) to low-speed even if the wait timed out, or
// pp->low_speed and the SM programs stay out of sync with the endpoint and EVERY
// following low-speed transaction is garbage (a low-speed device behind a hub is the
// only user of this path — direct-attached devices never send PRE).
#define PIO_USB_TX_WAIT_NORET(cond)                               \
  do {                                                            \
    uint32_t _t0 = get_time_us_32();                              \
    while (cond) {                                                \
      if (get_time_us_32() - _t0 > PIO_USB_TX_WAIT_US) {          \
        pio_usb_tx_timeouts++;                                    \
        break;                                                    \
      }                                                           \
    }                                                             \
...
static void __no_inline_not_in_flash_func(send_pre)(pio_port_t *pp) {
  ...
  PIO_USB_TX_WAIT_NORET((pp->pio_usb_tx->irq & IRQ_TX_EOP_MASK) == 0); // PICO-SPEC PATCH
  ...
  PIO_USB_TX_WAIT_NORET(!(pp->pio_usb_tx->fdebug & stall_mask));    // PICO-SPEC PATCH
...
void __not_in_flash_func(pio_usb_bus_usb_transfer)(pio_port_t *pp,
  ...
  PIO_USB_TX_WAIT((pp->pio_usb_tx->irq & IRQ_TX_ALL_MASK) == 0);   // PICO-SPEC PATCH
  pp->pio_usb_tx->irq = IRQ_TX_ALL_MASK; // clear complete flag
  ...
  if (pp->low_speed) {
    ...
    PIO_USB_TX_WAIT(*pc <= PIO_USB_TX_ENCODED_DATA_COMP);         // PICO-SPEC PATCH
  } else {
    PIO_USB_TX_WAIT(*pc < PIO_USB_TX_ENCODED_DATA_COMP);          // PICO-SPEC PATCH
  }
}
