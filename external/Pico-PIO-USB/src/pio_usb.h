...
// PICO-SPEC PATCH: packed internal state of one endpoint, for host-side diagnostics
// (see pio_usb_host.c). ~0 when the endpoint is not in the pool.
uint32_t pio_usb_host_ep_debug(uint8_t dev_addr, uint8_t ep_num);
// PICO-SPEC PATCH: re-derive the bus clock dividers after clk_sys changed
// (pico-speccy applies the saved Config::cpu_mhz after the host is up).
