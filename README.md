...
- OPL3 (YMF262) sound-card emulation on ports #C4-#C7 (Audio → "YMF262 (OPL3)") — together with AY and TSFM this covers the [VGM player plugin for the ESXDOS NMI browser](https://github.com/Alex-Zor/VGM-Plugin-for-DivMMC): copy the plugin to `/BIN/BPLUGINS` on the SD card and .vgm files (AY / YM2203 / OPL2 / OPL3) play from the NMI browser.
- CMS / Game Blaster (2x SAA1099 at 7.159 MHz) and 2x SN76489 (Sega arcade pair) emulation for the same VGM-card family — Audio → "2x SAA1099 (CMS)" / "2x SN76489". **Proposed port map** (no plugin drives these chips yet — this is the spec offered to the plugin): CMS on #D4-#D7 laid out like the PC original at 220h-223h (+0 chip-1 data, +1 chip-1 register, +2 chip-2 data, +3 chip-2 register, write-only), SN76489 #1 on #C9 (VGM command 0x50 — matches the VGM Player 0.61a build, which already plays single-SN76489 files) and #2 on #CD (command 0x30, proposed), write-only, standard 3.579545 MHz clock, Sega/VGM-default noise (16-bit LFSR, feedback bit0^bit3).
