...
## OPL3 (YMF262) + VGM plugin support (2026-08-28, NOT hw-tested)
...
- **`src/OplFm.{h,cpp}` is a port of MAME ymf262.cpp** (mame0220, GPL-2.0+,
  ...
  pairing, rhythm mode, tremolo/vibrato LFO, both timers. MAME's two runtime
  .bss tables (tl_tab 26 KB + sin_tab 32 KB) are **const flash**
  (`src/OplTabs.h`, generated by `tools/gen_opl_tabs.c` with byte-identical
  math — regenerate with that tool, never hand-edit). Object: ~41 KB flash,
  **4 B .bss** (the `oplfm` pointer); ~11.6 KB heap only while enabled
  (OplSubsys: ~9 KB chip state + 2x1280 B stereo buffers). Timers are Q16
  chip-sample countdowns advanced in gen() (MAME uses attotime callbacks);
  a whole-chip quiet fast path makes an idle enabled chip nearly free (safe
  because EG leaves OFF only on a key-on = a register write; no CSM on OPL3).
